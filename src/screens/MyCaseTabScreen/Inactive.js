...
class AllCaseTab extends Component {
    ...
    componentDidMount(){
        ...
        //     //             });
        //     //         }
        //     //         self.setState({caselist: filterCases})
        //     //     } else {
        //     //
        //     //     }
        //     // }).catch(err => {
        //     //     alert(JSON.stringify('gsdfg',err))
        //     // })
        //
        ...
        //                     });
        //                 }
        //                 const cases = filterCases.filter((item)=>{return item.status==='inactive'})
        //                 console.log('my inactive cases', cases);
        //
        ...
        if(!nextProps.mycaselist.error){
            ...
            if(nextProps.mycaselist.mycaselist.length > 0){
                if(this.props.mycaselist.mycaselist != nextProps.mycaselist.mycaselist){
                    ...
                    const cases = mycaselist.filter((item)=>{return item.status==='inactive'})
                    var activeCase;
                    if(nextProps.mycaselist.isRefresh){
                        ...
                        if(cases.length > 0){
                            if (caselist.filter(e => e.caseId === cases[0].caseId).length == 0) {
                                ...
                                console.log('new inactive Offset:',newOffset)
                            }else{
                                activeCase = caselist
                                newOffset = offset
                            }
                    ...
                    console.log('my inactive cases', activeCase);
                ...
                <FlatList
                    ...
                    renderItem = {(item,index) => {
                        return <CaseListItem cases={item} onPrayClick={this.onPrayClick} onNoofPrayerClick={this.noOfPrayerClick}
                                             isMyCases={true} tab={1} onImageClick={(arrImages) => {
