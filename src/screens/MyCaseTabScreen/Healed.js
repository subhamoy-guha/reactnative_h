...
class AllCaseTab extends Component {
    ...
    componentDidMount(){
        ...
        //                     });
        //                 }
        //                 const cases = filterCases.filter((item)=>{return item.status==='healed'})
        //                 console.log('my healed cases', cases);
        ...
        if(!nextProps.mycaselist.error){
            ...
            if(nextProps.mycaselist.mycaselist.length > 0){
                if(this.props.mycaselist.mycaselist != nextProps.mycaselist.mycaselist){
                    ...
                    const cases = mycaselist.filter((item)=>{return item.status==='healed'})
                    var activeCase;
                    if(nextProps.mycaselist.isRefresh){
                        ...
                        if(cases.length > 0){
                            if (caselist.filter(e => e.caseId === cases[0].caseId).length == 0) {
                                ...
                                newOffset =  offset + 1
                                console.log('new healed Offset:',newOffset)
                            }else{
                                activeCase = caselist
                                newOffset = offset
                            }
                    ...
                    console.log('my healed cases', activeCase);
    ...
    fetchData = () => {
        ...
        console.log('healed offset',offset)
        AsyncStorage.getItem('token').then((value) => {
            let formdata = new FormData();
            formdata.append("user_token", value);
                    ...
                    <FlatList
                        ...
                        renderItem = {(item,index) => {
                            return <CaseListItem cases={item} onPrayClick={this.onPrayClick} onNoofPrayerClick={this.noOfPrayerClick}
                                                 isMyCases={true} tab={2} onImageClick={(arrImages) => {
